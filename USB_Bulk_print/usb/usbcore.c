...
{ // Передача данных хосту 
  ...
  USART1_printfBuf("InStage Count=%d  ", ud->EP0Data.Count);
...
{ // Приём организован так, что недополученные данные (конечная точка имеет ограниченный буфер приёма) с прошлого приёма добавляются к уже полученным в текущем приёме
  ...
  USART1_printfBuf("OutStage Count=%d\n", ud->EP0Data.Count);
...
uint32_t USB_GetDescriptor(USBDevice_t *uhd) // Обработчик запроса GET_DESCRIPTOR 
{
  ...
  switch (uhd->sp.bmRequestType.BM.Recipient)
  {
    case REQUEST_TO_DEVICE:  // получатель запроса - устройство USB
                            ...
                            USART1_printfBuf("TO DEVICE  sp.wValue.WB.H=%d\n", uhd->sp.wValue.WB.H);

                            switch (uhd->sp.wValue.WB.H) 
                            {
                              case USB_DEVICE_DESCRIPTOR_TYPE:

                                                              USART1_printfBuf("DEVICE_DESCRIPTOR\n");
                              ...
                              case USB_CONFIGURATION_DESCRIPTOR_TYPE:
                                                                    USART1_printfBuf("CONFIGURATION_DESCRIPTOR\n");
                              ...
                              case USB_STRING_DESCRIPTOR_TYPE:
                                                              ...
                                                              USART1_printfBuf("STRING_DESCRIPTOR uhd->sp.wValue.WB.L=%d\n", uhd->sp.wValue.WB.L);
  ...
  USART1_printfBuf("ep.count=%d  len=%d\n", uhd->EP0Data.Count, len);
...
uint32_t USB_SetConfiguration(USBDevice_t *uhd) //
{
    ...
    while (((USBCommonDescriptor_t *)pD)->bLength) 
    {
      switch (((USBCommonDescriptor_t*)pD)->bDescriptorType) 
      {
        case USB_CONFIGURATION_DESCRIPTOR_TYPE: // Установка конфигурации в соответствии с тем номером, который получен в Setup пакете
                                              ...
                                              USART1_printfBuf("USB_CONFIGURATION_DESCRIPTOR_TYPE\n");
        ...
        case USB_INTERFACE_DESCRIPTOR_TYPE:
                                          ...
                                          USART1_printfBuf("USB_INTERFACE_DESCRIPTOR_TYPE\n");

                                          //alt = ((USBInterfaceDescriptor_t *)pD)->bAlternateSetting; // Определяем признак альтернативного интерфейса
                                          break;
        case USB_ENDPOINT_DESCRIPTOR_TYPE:

                                          USART1_printfBuf("USB_ENDPOINT_DESCRIPTOR_TYPE epn=%x \n", ((USBEndPointDescriptor_t *)pD)->bEndpointAddress);
  ...
  { // Если в конфигурационном пакете не задан номер конфигурации (нумеруется с 1), то
    ...
    USART1_printfBuf(" USB_SetConfiguration  OUT  ConfigOK = 0\n");

    return 0;
  }

  USART1_printfBuf(" USB_SetConfiguration  OUT  uhd->ConfigOK=%d\n", uhd->ConfigOK);
...
void USB_EP0_Setup_package(USBDevice_t *ud) // Процедура обрабатывает приход к USB устройству конфигурационного пакета
{
  int validep=0;

  USB_ReadEP(0x00, (uint8_t *)&ud->sp);      // Выполняем чтение конфигурационного покета
  ud->EP0Data.Count = ud->sp.wLength;       // Запоминаем кол-во байт, которое потом нужно будет принять

  USART1_printfBuf("Setup len=%d Recipient=%d Type=%d Dir=%d\n", ud->sp.wLength, ud->sp.bmRequestType.BM.Recipient, ud->sp.bmRequestType.BM.Type, ud->sp.bmRequestType.BM.Dir);

  switch(ud->sp.bmRequestType.BM.Type)
  {
    case REQUEST_STANDARD: // Если это стандартный запрос, то
                          switch (ud->sp.bRequest) // Проверка стандартных запросов к устройству
                          {
                            case USB_REQUEST_GET_DESCRIPTOR: // запрос к устройству для получения дескриптора (дескриптора устройства, кофигурации, конечной точки ...)
                                                            ...
                                                            USART1_printfBuf("SUP  GET_DESCRIPTOR\n");

                                                            if(!USB_GetDescriptor(ud)) validep = 1;  // USB_GetDescriptor() - передача хосту запрошенного дескриптора
                                                            break;
                            ...
                            case USB_REQUEST_SET_ADDRESS:  // Запрос для задания адреса USB устройства на шине
                                                           ...
                                                           // Адрес, который назначил хост USB устройство должно установить по приходу пакета IN
                                                            ...
                                                            USART1_printfBuf("SUP  SET_ADDRESS addr=%d\n", ud->sp.wValue.WB.L);

                                                            switch (ud->sp.bmRequestType.BM.Recipient)
                                                            {
                                                              case REQUEST_TO_DEVICE:
                                                                                      ud->Address = 0x80 | ud->sp.wValue.WB.L;  // Запоминаем адрес устройства на шине (с установкой 7-го, что будет означать, что хост назначил адрес USB устройству)
                                                                                      // непонятно по какой причине, но эти действия необходимо выполнить для всех IN точек, которые описаны в дескрипторе конфигурации
                            ...
                            case USB_REQUEST_SET_CONFIGURATION:  // Хост указывает устройству какую конфигурацию необходимо установить
                                                              ...
                                                              USART1_printfBuf("SUP  SET_CONFIGURATION\n");

                                                              switch (ud->sp.bmRequestType.BM.Recipient)
                                                              {
                                                                case REQUEST_TO_DEVICE:
                                                                                        if(USB_SetConfiguration(ud))
                                                                                        {
                                                                                          USB_WriteEP(0x80, ((void *)0), 0); // Эта запись означает, что передача данных не производится
                                                                                        }
                                                                                        else
...
void EndPoint0Event(uint32_t event) // Процедура обрабатывает пакет приходящий от Хоста (конфигурационный пакет, передача данных, приём данных, перевод точки в состояние STALL)
{
  ...
  switch (event)
  {
    ...
    case USB_EVT_OUT: // Если хост передаёт данные устройству
                      ...
                      USART1_printfBuf("EP0 OUT\n");

                      if(ud->sp.bmRequestType.BM.Dir == 0) // Данные приходят от хоста к USB устройству
                      {
                        if(ud->EP0Data.Count) // Если EP0Data.Count<>0, значит предыдущие данные были получены не полностью
                        {
                        	USB_EP0DataOutStage(ud);  // получаем очередную порцию данных
                          if(ud->EP0Data.Count == 0) // Если хост "говорит", что передаёт данные, но количество данных равно нулю, то
                          {
                            USB_SetNAK(0x80);
                            break;
                          }
                        ...
                        USB_ReadEP(0x00, ud->EP0Buf); // Получение статуса (USB_StatusOutStage();)
                      break;

    case USB_EVT_IN:  // Хост запрашивает данные у USB устройства

                    USART1_printfBuf("EP0 IN\n");
                    ...
                    if(ud->Address & 0x80)  // Если хост назначил адрес USB устройству, то этот адрес нужно задать в регистре USB
                    {
                      ...
                      USART1_printfBuf("USB Set addres %d\n", ud->Address);
